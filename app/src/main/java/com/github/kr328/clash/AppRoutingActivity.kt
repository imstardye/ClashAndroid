...
import com.github.kr328.clash.design.AppRoutingDesign
...
class AppRoutingActivity : BaseActivity<AppRoutingDesign>() {
    override suspend fun main() {
        ...
        defer {
            withContext(Dispatchers.IO) {
                val modeChanged = currentMode != service.accessControlMode
                val packagesChanged = selected != service.accessControlPackages
                
                service.accessControlMode = currentMode
                service.accessControlPackages = selected
                
                if (clashRunning && (modeChanged || packagesChanged)) {
                    stopClashService()
                    while (clashRunning) {
                        delay(200)
                    }
        ...
        val design = AppRoutingDesign(this, uiStore, selected, currentMode)

        setContentDesign(design)

        design.requests.send(AppRoutingDesign.Request.ReloadApps)
        ...
        while (isActive) {
            select<Unit> {
                ...
                design.requests.onReceive {
                    when (it) {
                        AppRoutingDesign.Request.ReloadApps -> {
                            currentMode = design.mode
                            design.patchApps(loadApps(selected))
                        }

                        AppRoutingDesign.Request.SelectAll -> {
                        ...
                        AppRoutingDesign.Request.SelectNone -> {
                            selected.clear()

                            design.rebindAll()
                        }

                        AppRoutingDesign.Request.SelectInvert -> {
                            val all = withContext(Dispatchers.Default) {
                        ...
                        AppRoutingDesign.Request.Import -> {
                        ...
                        AppRoutingDesign.Request.Export -> {
                            val clipboard = getSystemService<ClipboardManager>()
